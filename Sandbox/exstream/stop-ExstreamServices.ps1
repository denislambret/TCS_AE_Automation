...
PROCESS { 
    ...
    foreach ($service in $services_list) {
      ...
      Log -Level 'INFO' -Message ("Stoping service " + $service.name + " - " + $service.description)
      Log -Level 'INFO' -Message $SEP_L2
      $attempts = 0
      $countSrv = (Get-Service $service.name | ? {$_.status -match "Stopped"}).count  
      if ($countSrv -gt 0) {
        Log -Level 'INFO' -Message ("Service already stopped...")
        continue
      }
      
      while (($attempts -le $service.retry) -and ($countSrv -eq 0)) {
            Log -Level 'DEBUG' -Message ("Stoping Service " + $service.name)   
