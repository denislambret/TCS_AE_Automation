...
BEGIN {
    ...
    Set-DefaultLogLevel -Level "INFO" | Out-Null
    Set-MinLogLevel -Level "DEBUG" | Out-Null
}
    ...
    $attempts = 0
      ...
      while (($attempts -le $service.retry) -and ($countSrv -eq 0)) {
            if (($service.defaultStatus -eq "Stopped") -or ($force))   {
                ...
                Log -Level 'WARNING' -Message ("Service default constraint (" + $service.defaultStatus + ") prevents stop action.")
                break
            }
