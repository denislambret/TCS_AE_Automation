...
PROCESS {
    ...
    if ($date) {
        ...
        $ctrDate = get-date -DisplayHint date
    }
    ...
    $countFiles = (Get-ChildItem -path $source_scanfile\*.csv | where-object {$_.LastWrtieTime -gt (get-date).AddDays(-1)}).Count
    $countFile
    $countLinks = (Get-ChildItem -path $source_scanfile\*.csv | where-object {$_.LastWrtieTime -gt (get-date).AddDays(-1)} | Get-Content).Count
