...
PROCESS {
    ...
    $VERSION      = '1.0'
    $AUTHOR       = 'DLA'
    $SCRIPT_DATE  = '20230126'

    # Initialize SFTP parameters for connection
    $root_script  = 'Y:\03_DEV\06_GITHUB\tcs-1\Projects\SAP_AvancePay'
    $session      = 0;
    ...
    function Connect-SFTPPrivKey {
        ...
        $session = New-SFTPSession -ComputerName $conf.conf.sftp_server.computername -Credential $credential -KeyFile $conf.conf.sftp_server.privkey -ErrorVariable $err
        ...
        if ($fileList) {
            $fileList | ForEach-Object {
                try {
                    ...
                    #Get-SFTPFile -SessionId $session.SessionID -LocalPath ($conf.conf.sftp_server.sftp_success_path + '/' + $_.name) -RemoteFile $conf.conf.pathes.local_path -overvrite
    ...
    function helper {
        ...
        '-send      Send email communication by script / not VTOM'
    ...
    $countFiles = 0;
    $countFiles = (Test-SFTPFiles -path $sftp_input_path).Count
    if($countInputFiles) {
    ...
    $subject = "CTR Avance Paie OK - "+(get-date -f "yyyMMdd")
    $body = "Le contrôle des avances paiements s'est correctement terminé.\nVeuillez prendre connaissance des logs de traitements en attachement de ce message";
    
    Log -Level 'INFO' -Message ("Mail send : " + $subject)
    $reportLog = ($global:log_path + "\" + $global:log_name)
    Copy-Item -path $reportLog -destination $conf.conf.pathes.data_path
    #Send-Message -body $body -subject $subject -attachment ($conf.conf.pathes.data_path + "\" + $global:log_name)
