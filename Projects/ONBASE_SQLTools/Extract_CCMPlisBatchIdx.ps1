#----------------------------------------------------------------------------------------------------------------------------------
# Script  : Extract_CCMPlisBatchIdx.ps1
#----------------------------------------------------------------------------------------------------------------------------------
# Author  : DLA
# Date    : 20230627
# Version : 0.1
...
BEGIN {
    ...
    $script_path      = $global:ScriptRoot + "\Projects\ONBASE_SQLTools"
...
PROCESS {
    ...
    $outputFileName = $conf.conf.output_path + "\" + $date + "_ExtractBatchIdx_"+ $batch +".csv" 
    $query = @" 
    ...
    itemdate, 
    datestored,
    KI110.keyvaluechar AS CCM_CustRefNumber
    FROM hsi.itemdata ITDATA
    JOIN hsi.keyitem110 KI110 ON (ITDATA.itemnum = KI110.itemnum) 
    WHERE itemname LIKE '%CCM Pli%'
    AND batchnum != 0
    AND batchnum = $batch
"@
