...
ForEach ($folder in $allFolders) {
    ...
    $folderSizeInKB = "{0} KB" -f [Math]::Round(($folderSize.Sum / 1KB),0)
    $folderSizeInMB = "{0} MB" -f [Math]::Round(($folderSize.Sum / 1MB),2)
    $folderSizeInGB = "{0} GB" -f [Math]::Round(($folderSize.Sum / 1GB),3)
...
if ($AddTotal) {
    ...
    if ($folderList.Count -gt 1) {
        ...
        $totalFolderSizeInKB = "{0} KB" -f [Math]::Round(($grandTotal / 1KB),0)
        $totalFolderSizeInMB = "{0} MB" -f [Math]::Round(($grandTotal / 1MB),2)
        $totalFolderSizeInGB = "{0} GB" -f [Math]::Round(($grandTotal / 1GB),3)
